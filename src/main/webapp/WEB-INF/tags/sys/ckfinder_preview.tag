...
<ol id="${input}Preview" style="margin-top: -3px;margin-left: -3px"></ol>
<script type="text/javascript">
	...
	function ${input}Preview(){
		...
		var urlImg = $("#nameImage").attr("value");
		if(urlImg != null && urlImg != "" ){
			var url = "<c:url value='/updateUserImgUrl'/>";
			$.post(url,"urlImg="+urlImg,function(data){
				return;
			});
			//window.location.href="${front}/updateUserImgUrl?urlImg="+urlImg+"";
		}				
	}
	${input}Preview();
</script>