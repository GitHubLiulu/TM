...
<head>
	...
	<style type="text/css">
		...
		.hntop-nav-lst{width:162px;display:block;position:absolute;top:50px;z-index:9999;height:auto;border:1px solid #e8e8e8;/* border-top:none; */border-bottom:none;}
		.hntop-nav-lst dl{float:left;line-height:35px;width:162px}
		.hntop-nav-lst dl dt{padding-left:10px;color:#666;height:35px;background:#fff;border-bottom:1px solid #e8e8e8;}
		.hntop-nav-lst dl dt a{float:right;/* margin:7px 6px 0 10px; */padding:0 4px;border:1px solid #e8e8e8;height:20px;line-height:20px;color:#666;}
	    .hntop-nav-lst dl dt a:hover{border:1px solid #4cd386;color:#4cd386}
	...
	<script type="text/javascript">
		...
		function queryRemindDate(){
			var url = "<c:url value='a/tm/sysToolCheckRecord/queryRemindList'/>";
			var html = "";
			var count = 0;
			$.post(url,function(data){
				$.each(data,function(i,obj){
				    count++;
                   /* html += "<li><a href=\"${ctx}/tm/sysToolAll/checkForm?id="+obj.toolId+"\" target=\"mainFrame\">工具:"+obj.toolId+"&nbsp; 校验</a></li> "*/
				});
				html = "<li>&nbsp;您需要校验工具有："+count+"个</li>";
				if(html==""){
					html = "<li>您暂无需要校验的工具！</li>";
				}
				$("#dataRemind").html(html);
			});
		}
	</script>
</head>
	...
	<div id="main">
		<div id="header" class="navbar navbar-fixed-top">
			<div class="navbar-inner">
				...
				<ul id="userControl" class="nav pull-right">
					<li><a href="${pageContext.request.contextPath}${fns:getAdminPath()}/index" target="_blank" title="访问网站主页"><i class="icon-home"></i></a></li>
					...
					<li id="userInfo" class="dropdown">
						...
						<%--<li class="hntop-nav-lst">
	 						<dl id="dataRemind">

	 						</dl>
	 					</li>--%>
						<ul class="dropdown-menu" id = "dataRemind" style="display: block">
						</ul>
						<%-- <ul class="dropdown-menu">
	    ...
	    <div class="container-fluid">
		    ...
		    <div id="footer" class="row-fluid">
	            Copyright &copy; ${fns:getConfig('copyrightYear')} ${fns:getConfig('productName')} - Powered By <a href="http://ecsite.com" target="_blank">ecsite</a> ${fns:getConfig('version')}
			</div>
		</div>
	</div>
	<script type="text/javascript"> 
