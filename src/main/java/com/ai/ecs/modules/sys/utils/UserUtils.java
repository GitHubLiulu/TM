...
public class UserUtils {
	 ...
	 * @return 取不到返回null
	 */
	public static User setUser2Cache(User user){
	...
	public static List<Menu> getAdminMenuList(){
		List<Menu> menuList = null;
		if (menuList == null){
			User user = getUser();
			if (user.isAdmin()){
				menuList = menuDao.findAdminAllMenu(new Menu());
			}else{
				Menu m = new Menu();
				m.setUserId(user.getId());
				menuList = menuDao.findByAdminUserId(m);
			}
			//putCache(CACHE_MENU_LIST, menuList);
		}
		return menuList;
	...
	public static List<Area> queryAreaTop() {
		return areaDao.queryAreaTop();
	}
